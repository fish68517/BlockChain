...
class AllBids extends React.Component {
  ...
  // change the status of bid listing once it gets selected
    ...
    if(window.ethereum) {
      ...
      await setRestorerCCProjectListing(projectAddress, restorerAddress, biddingPrice);
    } else {
  ...
  render() {
    ...
    return (
      <div className="container">
        <MetaConnectModal showModal={this.state.showModal} setShowModal={this.setShowModal}/>
...
function BidListingList(props) {
  ...
  const listingSections = Object.keys(projectListingToBidsMapping).map(key => {
...
function BidListing({ bid, handleSelectBtn, projectListing, hasSelectedBid }) {
  const modalBody = (
    <div>
      Selecting a bid for {projectListing.make} {projectListing.model}. The bid price is going to be set as the funding goal for investors.
      Are you sure you want to approve a bid of {bid.biddingPrice} from restorer {bid.bidderId}?
    </div>
  );
        ...
        {bid.isSelected ?
          ...
          !hasSelectedBid ?
            <Modal
              ...
              onSubmit={() => handleSelectBtn(bid.id, bid.biddingPrice, bid.restorerAddress, projectListing.projectAddress)}
            /> :
            null
        }
      </td>
