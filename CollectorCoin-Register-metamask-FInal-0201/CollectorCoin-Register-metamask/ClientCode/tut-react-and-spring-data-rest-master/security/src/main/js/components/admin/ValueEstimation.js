...
function ValueEstimation({ listing, isEditable, saveEstimations }) {
  const [valueEstimation, setValueEstimation] = useState(listing?.valueEstimation || '');
  const [repairCost, setRepairCost] = useState(listing?.repairCostEstimation || '');
  ...
  return (
    <div>
      ...
      {isEditable &&
        <button className="btn btn-primary d-inline-block mt-1" onClick={onSubmitEstimations}>
          Save Estimations
        </button>
      }
      {message &&
        <div className={`d-inline-block mt-2 p-2 alert alert-${error ? 'error' : 'success'}`}>
          {message}
        </div>
      }
      </div>
