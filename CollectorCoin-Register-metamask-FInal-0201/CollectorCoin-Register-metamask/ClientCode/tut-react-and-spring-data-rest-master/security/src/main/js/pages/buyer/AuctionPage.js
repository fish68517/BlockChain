...
const { bidForCCProjectListing, getWalletAddress } = require('../../utils/web3Utils');
...
const {
  ...
  getAuctionBidListings,
  updateAuctionBidById,
} = require('../../actions/AuctionBid');

function AuctionPage({ user, dispatch, listings, bids }) {
  ...
  const [showModal, setShowModal] = useState(false);
	...
	useEffect(() => {
    setIsLoading(true);
    const getListings = () => dispatch(getListingsPendingAuctionBid());
    const getBids = () => dispatch(getAuctionBidListings(user.id));
  ...
  const updateBid = (bidId, listingId, bidPrice) => {
    if (bidPrice <= 0) // invalid bid
      ...
      setError('');
      dispatch(updateAuctionBidById(bidId, { buyerPrice: bidPrice }))
      .then(() => {
        setError('');
  ...
  const createBid = async (listingId, bidPrice, projectAddress) => {
    let walletAddress;
    if (bidPrice <= 0) // invalid bid
      setError("Bid value must be greater than 0");
    else {
      if(window.ethereum) {
        ...
        await bidForCCProjectListing(projectAddress, bidPrice);
      } else {
        setShowModal(true);
        setError(`Error creating a bid for listing ${listingId} in contract`);
      }

      setError('');
      dispatch(createAuctionBidListingByBuyerID(user.id, { listingId, buyerPrice: bidPrice, buyerAddress: walletAddress }))
      .then(() => {
        setError('');
        setStatus(`Successfully created a bid for listing ${listingId}.`)
      }).catch(e => {
...
const NewBidRow = ({ projectListing, existingBid, onCreate, onUpdate }) => {
  const [bidPrice, setBidPrice] = useState(existingBid ? existingBid.buyerPrice : 0);
    ...
    <tr key={projectListing.id}>
      ...
      <td>
        ...
        {existingBid ?
          ...
          <Button type="button" id="bidbtn" onClick={() => onCreate(projectListing.id, bidPrice, projectListing.projectAddress)}>
            Bid
          </Button>
        }
      </td>
...
function mapStateToProps({ auth, auctionBidListings, projectListings }) {
  return {
    user: auth.user,
    listings: projectListings,
    bids: auctionBidListings,
  }
}

module.exports = connect(mapStateToProps)(AuctionPage);
