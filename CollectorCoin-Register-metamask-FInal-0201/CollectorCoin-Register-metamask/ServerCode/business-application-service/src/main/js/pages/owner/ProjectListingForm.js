...
const ProjectListingForm = ({ listing, onSubmit, onError, onFileClick }) => {
  ...
  const isFormValid = () => {
    if (!make || !model || !vin || !ccpg || !fundingGoal) {
  ...
  return (
    <Form>
      ...
      <Form.Group>
        <Row className="justify-content-md-center">
          <Col xs={6}>
            <Form.Label htmlFor="ccpg">
              Collector Car Price Guide Value
            </Form.Label>
            <Form.Control
              required
          ...
          <Col xs={6}>
            ...
            {listing && (
              <div className="text-sm">
                ...
                <button className="btn btn-link p-0" onClick={onFileClick}>
      ...
      <div>
        <Button variant="primary" type="submit" onClick={createProjectListing}>
          {listing ? "Save" : "Submit Project"}
        </Button>
      </div>
    </Form>
  );
