...
public class InvestmentService {
  ...
  /**
   * Create investment with blockchain transaction
   * 
   * @param investment The investment data
   * @param userId The user ID making the investment
   * @param transactionHash The blockchain transaction hash (for logging/tracking)
  ...
  public Investment createInvestmentWithTransaction(Investment investment, Long userId, String transactionHash) {
    if (transactionHash == null || transactionHash.isEmpty()) {
      logger.error("Transaction hash is required for investment tracking");
      throw new RuntimeException("Transaction hash is required");
    }
    
    logger.info("Processing investment with transaction hash: {}", transactionHash);
    
    // Update jBPM and create investment
