...
function AuctionPage({ user, dispatch, listings, bids }) {
  ...
  const createBid = async (listingId, bidPrice, projectAddress) => {
    ...
    if (bidPrice <= 0)
      ...
      setError("");
      dispatch(
        createAuctionBidListingByBuyerID(user.id, {
          listingId,
          buyerPrice: bidPrice,
  ...
  return (
    <div className="container">
      ...
      {status && (
        <div className="d-inline-block mt-2 p-2 alert alert-success">
          {status}
        </div>
      )}
      {error && (
      ...
      {!isLoading && bids.length === 0 && (
        <div className="d-inline-block mt-2 p-2 alert alert-info">
          ...
          <tbody>
            {listings.map((projectListing) => {
              const existingBid = bids.find(
                (b) => b.listingId === projectListing.id
              );
              return (
                <NewBidRow
...
const NewBidRow = ({ projectListing, existingBid, onCreate, onUpdate }) => {
  const [bidPrice, setBidPrice] = useState(
    existingBid ? existingBid.buyerPrice : 0
  );
      ...
      <td>
        ...
        {existingBid ? (
          <Button
            variant="info"
            onClick={() =>
              onUpdate(existingBid.id, projectListing.id, bidPrice)
            }
          >
            Re-bid
          </Button>
