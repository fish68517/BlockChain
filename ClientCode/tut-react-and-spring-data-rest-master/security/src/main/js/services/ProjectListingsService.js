...
const getAllProjectListings = () => {
	return createBaseRequest().get("/all-projectListings-detail")
};
...
const getProjectListingsByUserID = (userID, size, number) => {
  ...
  return createBaseRequest().get(`/users/id/${userID}/projectListings`, { params });
};

const createProjectListingByUserID = (userID, data) => {
  return axios.post(`http://localhost:8080/api/users/${userID}/projectListings`, data, {
    headers: {
...
const editProjectListing = (listingId, data) => {
  return axios.put(`http://localhost:8080/api/projectListings/${listingId}`, data, {
    headers: {
      'Content-Type': 'multipart/form-data',
      ...getAuthHeader(),
    }
...
const addValueEstimation = (projectListingID, data) => {
  return createBaseRequest().post(`/projectListings/${projectListingID}/valueEstimation`, data);
}
...
const assignRestorationById = (listingId) => {
  return createBaseRequest().post(`/projectListings/${listingId}/assignRestoration`);
}

const finishRestorationById = (listingId) => {
  return createBaseRequest().post(`/projectListings/${listingId}/finishRestoration`);
}

const postItemForSaleById = (listingId) => {
  return createBaseRequest().post(`/projectListings/${listingId}/postItemForSale`);
}
...
//const findByVIN = vin => {
//  return http.get(`/projectListings?vin=${vin}`);
//};
