...
const CCTOKEN_ADDRESS = "0x60c5eb023F7778031F542ece23dff7165a515CC0";
const FACTORY_ADDRESS = "0xc2B45f7DfCf4Fb869133cC35BEdaA9c63D95Fda3";
...
const getCCTokenBalance = async () => {
    try {
        ...
        const ccTokenContract = new ethers.Contract(CCTOKEN_ADDRESS, CCTOKEN_ABI, signer);
...
const approveFunding = async (projectAddress, amount) => {
    try {
        ...
        const signer = provider.getSigner();
        const ccTokenContract = new ethers.Contract(CCTOKEN_ADDRESS, CCTOKEN_ABI, signer);
        const tx =  await ccTokenContract.increaseAllowance(projectAddress, appendZeros(amount, 18));
        await tx.wait();
        console.log(tx.hash);
    } catch(ex) {
        console.log(ex);
...
const saveEstCCProjectListing = async (projectAddress, valueEstimate, repairEstimate) => {
    try {
        await window.ethereum.request({ method: 'eth_requestAccounts' });
        const provider = await new ethers.providers.Web3Provider(window.ethereum, "any");
        provider.send('eth_requestAccounts', []);
...
const setRestorerCCProjectListing = async (projectAddress, restorerAddress, fundingGoal) => {
    try {
        await window.ethereum.request({ method: 'eth_requestAccounts' });
        const provider = await new ethers.providers.Web3Provider(window.ethereum, "any");
        provider.send('eth_requestAccounts', []);
...
const assignRestorationCCProjectListing = async (projectAddress) => {
    try {
        ...
        const tx = await factoryContract.assignForRestoration(
            projectAddress,
        );
        await tx.wait();
...
const openAuctionCCProjectListing = async (projectAddress) => {
    try {
        ...
        const tx = await factoryContract.setAuctionOpen(
            projectAddress,
        );
        await tx.wait();
...
const setBuyerCCProjectListing = async (projectAddress, buyerAddress) => {
    try {
        ...
        const tx = await factoryContract.setBuyer(
            projectAddress,
            buyerAddress
        );
        await tx.wait();
...
const redistributeCCProjectListing = async (projectAddress) => {
    try {
        ...
        const tx = await factoryContract.redistribute(
            projectAddress
        );
        await tx.wait();
...
const investInCCProjectListing = async (projectAddress, amount) => {
    try {
        ...
        provider.send('eth_requestAccounts', []);
        const signer = provider.getSigner();
        const projectContract = new ethers.Contract(projectAddress, PROJECT_ABI, signer);

        const tx = await projectContract.acceptFunds(
...
const bidForCCProjectListing = async (projectAddress, amount) => {
    try {
        ...
        const tx = await projectContract.addNewAuctionBid(
            appendZeros(amount, 18)
        );
        await tx.wait();
