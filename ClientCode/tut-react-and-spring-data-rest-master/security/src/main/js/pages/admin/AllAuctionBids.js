...
class AllAuctionBids extends React.Component {
  ...
  render() {
      ...
      <SelectedAuctionBidListings
        auctionBidListings={this.props.auctionBidListings.filter(bid => bid.isSelected)}
    ...
    return (
      <div className="container">
        <MetaConnectModal setShowModal={this.setShowModal} showModal={this.state.showModal} />
...
function AuctionBidListingList(props) {
  ...
  props.projectListings.forEach(listing => {
    projectListingToAuctionBidsMapping[listing.id] = {
      listing,
      bids: props.auctionBidListings.filter(bid => bid.listingId === listing.id),
    }
  })
  ...
  const listingSections = Object.keys(projectListingToAuctionBidsMapping).map(key => {
    ...
    return (
      <div className="mt-2" key={key}>
        <h4>Bids for {listing.make} {listing.model}({listing.vin})</h4>
...
function AuctionBidListing({ bid, handleSelectBtn, projectListing, hasSelectedBid }) {
  const modalBody = (
    <div>
      Selecting a auction bid for {projectListing.make} {projectListing.model}.
      Are you sure you want to approve a bid of {bid.buyerPrice} from buyer {bid.buyerId}?
    </div>
  );
        ...
        {bid.isSelected ?
          ...
          !hasSelectedBid ?
            <Modal
              ...
              onSubmit={() => handleSelectBtn(bid.id, bid.buyerAddress, projectListing.projectAddress)}
            /> :
            null
        }
      </td>
...
const SelectedAuctionBidListings = ({ auctionBidListings, projectListings }) => {
  return (
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {auctionBidListings.map(bid => {
          const projectListing = projectListings.find(p => p.id === bid.listingId);
          return (
            <tr key={bid.id}>
