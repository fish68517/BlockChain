...
class AdminPortal extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      attributes: [], page: 1, pageSize: 1, content: "ADMIN CONTENT",
...
const ApprovedProjectListings = (props) => {
	const projectListings = props.projectListings.filter(
		(projectListing) => projectListing["receiveTitle"] && projectListing["verifyDetails"]
	);

	return (
		<>
			{projectListings.map(projectListing =>
        <ApprovedProjectListing key={projectListing.id} projectListing={projectListing} />
      )}
		</>
	)
...
const ApprovedProjectListing = ({ projectListing }) => {
	return (
		<Card>
			<Card.Body>
				<Card.Title>
          <Link to={`/listings/${projectListing.id}`}>{projectListing.make} {projectListing.model}</Link>
        </Card.Title>
				<Card.Text>
					<small><strong>CCPG:</strong> {projectListing.ccpg}</small><br />
					<small><strong>Funding Goal:</strong> {projectListing.fundingGoal}</small><br />
					<small><strong>Funding Progress:</strong> {projectListing.currFundingAmount}</small><br />
				</Card.Text>
			</Card.Body>
		</Card>
	);
