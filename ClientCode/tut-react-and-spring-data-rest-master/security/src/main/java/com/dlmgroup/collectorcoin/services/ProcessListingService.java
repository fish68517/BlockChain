...
public class ProcessListingService {
  ...
  public List<ProjectListing> convertProcessResponseToProjectListing(List<ProcessResponse> list) {
    ...
    for (ProcessResponse r : list) {
      ...
      // ProjectListing l = projectListingRepository.findByProcessId(r.getProcessInstanceId())
        // .orElseThrow(() -> new ResourceNotFoundException("Listing not found with id = " + r.getProcessInstanceId()));
